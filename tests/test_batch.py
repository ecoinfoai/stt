...
class TestParseYamlList:
    ...
    def test_parse_yaml_list_string_items(self):
        ...
        items = batch.parse_yaml_list(text)
        assert [item.name for item in items] == ["강연 A", "강연 B"]
    ...
    def test_parse_yaml_list_bare_sequence(self):
        ...
        items = batch.parse_yaml_list("- 강연 A\n- 강연 B\n")
        assert len(items) == 2
    ...
    def test_parse_yaml_list_unknown_option(self):
        ...
        with pytest.raises(ValueError, match="quality"):
            batch.parse_yaml_list(text)

    def test_parse_yaml_list_empty(self):
        """항목이 없으면 ValueError."""
        with pytest.raises(ValueError):
            batch.parse_yaml_list("items: []\n")
...
class TestLoadList:
    ...
    def test_load_list_txt(self, tmp_path):
        ...
        assert len(batch.load_list(list_file)) == 1
    ...
    def test_load_list_missing(self, tmp_path):
        ...
        with pytest.raises(FileNotFoundError):
            batch.load_list(tmp_path / "none.txt")
...
class TestResolveName:
    ...
    def test_resolve_name_prefers_audio(self, tmp_path):
        ...
        assert batch.resolve_name("강연 A", tmp_path) == audio
...
class TestResolveItems:
    ...
    def test_resolve_items_success(self, tmp_path):
        ...
        items = batch.parse_txt_list("A\nB\n")
        resolved = batch.resolve_items(items, tmp_path)
    ...
    def test_resolve_items_reports_all_failures(self, tmp_path):
        ...
        with pytest.raises(FileNotFoundError) as excinfo:
            batch.resolve_items(items, tmp_path)
...
class TestBuildArgv:
    ...
    def test_build_argv_includes_paths_and_defaults(self):
        ...
        argv = batch.build_argv(
            [Path("A.m4a")], dict(batch.OPTION_DEFAULTS), None, False
        )
        assert argv[0] == "A.m4a"
    ...
    def test_build_argv_flags(self):
        """참/거짓 옵션은 플래그로만 붙는다."""
        options = dict(
            batch.OPTION_DEFAULTS, srt=True, no_timestamps=True
        )
        argv = batch.build_argv(
    ...
    def test_build_argv_includes_terms(self):
        ...
        options = dict(batch.OPTION_DEFAULTS, terms="terms.txt")
        argv = batch.build_argv([Path("A.m4a")], options, None, False)
...
class TestRunGroups:
    ...
    def test_run_groups_calls_transcribe(self, monkeypatch):
        ...
        calls: list[list[str]] = []
        monkeypatch.setattr(
            batch.transcribe, "main", lambda argv: calls.append(argv)
        )
        base = dict(batch.OPTION_DEFAULTS)
        groups = [(base, [Path("A.m4a")]), (base, [Path("B.m4a")])]
        assert batch.run_groups(groups, None, False, False) == []
        assert len(calls) == 2
    ...
    def test_run_groups_raises_without_keep_going(self, monkeypatch):
        ...
        monkeypatch.setattr(batch.transcribe, "main", boom)
        groups = [(dict(batch.OPTION_DEFAULTS), [Path("A.m4a")])]
        with pytest.raises(RuntimeError):
            batch.run_groups(groups, None, False, False)
    ...
    def test_run_groups_collects_failures(self, monkeypatch):
        ...
        base = dict(batch.OPTION_DEFAULTS)
        groups = [(base, [Path("A.m4a")]), (base, [Path("B.m4a")])]
        failures = batch.run_groups(groups, None, False, True)
...
class TestMain:
    ...
    def test_main_dry_run(self, tmp_path, monkeypatch, capsys):
        ...
        list_file.write_text("강연 A\n", encoding="utf-8")
        monkeypatch.setattr(
            batch.transcribe, "main", lambda argv: pytest.fail("실행됨")
        )
        code = batch.main([
...
class TestParseArgs:
    ...
    def test_parse_args_defaults(self):
        ...
        args = batch.parse_args(["--list", "list.txt"])
